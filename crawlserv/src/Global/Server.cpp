...
// constructor
crawlservpp::Global::Server::Server(const crawlservpp::Struct::DatabaseSettings& databaseSettings,
	...
	connection = mg_bind(&(this->eventManager), serverSettings.port.c_str(), crawlservpp::Global::Server::eventHandler);
	if(!connection) {
	...
	for(auto i = threads.begin(); i != threads.end(); ++i) {
		if(i->module == "crawler") {
			...
			*/
			...
			/*
			crawlservpp::Module::Analyzer::Thread * analyzer =
					new crawlservpp::Module::Analyzer::Thread(this->database, i->id, i->status, i->paused, i->options, i->last);
...
crawlservpp::Global::Server::~Server() {
	...
	this->database.log("server", "Shutting down after up-time of "
			+ crawlservpp::Helper::DateTime::secondsToString(this->getUpTime()) + ".");
}

// server tick
bool crawlservpp::Global::Server::tick() {
...
const std::string& crawlservpp::Global::Server::getStatus() const {
	return this->status;
}

// get up-time of server in seconds
unsigned long crawlservpp::Global::Server::getUpTime() const {
...
std::string crawlservpp::Global::Server::cmd(const char * body, struct mg_connection * connection, bool& threadStartedTo) {
	std::string ip = crawlservpp::Global::Server::getIP(connection);
	crawlservpp::Global::Server::CmdResponse response;

	// parse JSON
	rapidjson::Document json;
	if(json.Parse(body).HasParseError()) response = crawlservpp::Global::Server::CmdResponse(true, "Could not parse JSON.");
	else {
		// get server command
					...
					//TODO: server commands for analyzer
						...
						// create a worker thread for testing query (so large queries do not block the server)
						{
							...
							this->workers.push_back(
									new std::thread(&crawlservpp::Global::Server::cmdTestQuery, this, this->workers.size(), body, connection)
							);
						}
...
void crawlservpp::Global::Server::setStatus(const std::string& statusMsg) {
	this->status = statusMsg;
}

// handle server events
void crawlservpp::Global::Server::eventHandler(mg_connection * connection, int event, void * data) {
...
bool crawlservpp::Global::Server::cmdCheckNameSpace(const std::string& nameSpace) {
...
std::string crawlservpp::Global::Server::getIP(mg_connection * connection) {
...
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdKill(const rapidjson::Document& json, const std::string& ip) {
	...
	if(json.HasMember("confirmed")) {
		...
		// send bye message
		return crawlservpp::Global::Server::CmdResponse("Bye bye.");
	}

	return crawlservpp::Global::Server::CmdResponse(false, true, "Are you sure to kill the server?");
}

// server command allow(ip): allow acces for the specified IP(s)
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdAllow(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("ip"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'ip\' is missing).");
	if(!json["ip"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'ip\' is not a string).");
	std::string toAllow = json["ip"].GetString();

	// allow needs to be confirmed
	if(!json.HasMember("confirmed")) {
		return crawlservpp::Global::Server::CmdResponse(false, true, "Do you really want to allow " + toAllow + " access to the server?");
	}
	...
	return crawlservpp::Global::Server::CmdResponse("Allowed IPs: " + this->allowed + ".");
}

// server command disallow: revoke access from all except the initial IP(s) specified by the configuration file
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDisallow(const rapidjson::Document& json, const std::string& ip) {
	// reset alled IP(s)
	...
	return crawlservpp::Global::Server::CmdResponse("Allowed IP(s): " + this->allowed + ".");
}

// server command log(entry): write a log entry by the frontend into the database
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdLog(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("entry"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'entry\' is missing).");
	if(!json["entry"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'entry\' is not a string).");
	std::string entry = json["entry"].GetString();

	// write log entry
	this->database.log("frontend", entry);
	return crawlservpp::Global::Server::CmdResponse("Wrote log entry: " + entry);
}
...
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdClearLog(const rapidjson::Document& json, const std::string& ip) {
	// check whether the clearing of logs is allowed
	if(!(this->settings.logsDeletable))return crawlservpp::Global::Server::CmdResponse(true, "Not allowed.");
	...
	if(!json.HasMember("confirmed")) {
		...
		return crawlservpp::Global::Server::CmdResponse(false, true, replyStrStr.str());
	}
	...
	if(module.length()) {
		...
		return crawlservpp::Global::Server::CmdResponse("Logs of " + module + " cleared.");
	}

	this->database.log("server", "All logs cleared by " + ip + ".");
	return crawlservpp::Global::Server::CmdResponse("All logs cleared.");
}
...
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdStartCrawler(const rapidjson::Document& json,
		const std::string& ip) {
	// get arguments
	Struct::ThreadOptions options;
	if(!json.HasMember("website"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'website\' is missing).");
	if(!json["website"].IsInt64())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'website\' is not a number).");
	options.website = json["website"].GetInt64();

	if(!json.HasMember("urllist"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'urllist\' is missing).");
	if(!json["urllist"].IsInt64())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'urllist\' is not a number).");
	options.urlList = json["urllist"].GetInt64();

	if(!json.HasMember("config"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'config\' is missing).");
	if(!json["config"].IsInt64())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'config\' is not a number).");
	...
	if(!(this->database.isWebsite(options.website))) {
		...
		return crawlservpp::Global::Server::CmdResponse(true, errStrStr.str());
	}
	...
	return crawlservpp::Global::Server::CmdResponse("Crawler has been started.");
}

// server command pausecrawler(id): pause a crawler by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdPauseCrawler(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is not a number).");
	...
	return crawlservpp::Global::Server::CmdResponse("Crawler is pausing.");
}

// server command unpausecrawler(id): unpause a crawler by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdUnPauseCrawler(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Crawler is unpausing.");
}

// server command stopcrawler(id): stop a crawler by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdStopCrawler(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Crawler stopped.");
}

// server command startparser(website, urllist, config): start a parser using the specified website, URL list and configuration
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdStartParser(const rapidjson::Document& json, const std::string& ip) {
	// get arguments
	Struct::ThreadOptions options;
	if(!json.HasMember("website"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'website\' is missing).");
	...
	return crawlservpp::Global::Server::CmdResponse("Parser has been started.");
}

// server command pauseparser(id): pause a parser by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdPauseParser(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Parser is pausing.");
}

// server command unpauseparser(id): unpause a parser by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdUnPauseParser(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Parser is unpausing.");
}

// server command stopparser(id): stop a parser by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdStopParser(const rapidjson::Document& json, const std::string& ip) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Parser stopped.");
}

// server command resetparsingstatus(urllist): reset the parsing status of a id-specificed URL list
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdResetParsingStatus(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("urllist"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'urllist\' is missing).");
	if(!json["urllist"].IsInt64())
	...
	if(!json.HasMember("confirmed"))
		return crawlservpp::Global::Server::CmdResponse(false, true, "Are you sure that you want to reset the parsing status of this URL list?");

	// reset parsing status
	this->database.resetParsingStatus(listId);
	return crawlservpp::Global::Server::CmdResponse("Parsing status reset.");
}

// server command resetextractingstatus(urllist): reset the parsing status of a id-specificed URL list
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdResetExtractingStatus(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("urllist"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'urllist\' is missing).");
	if(!json["urllist"].IsInt64())
	...
	if(!json.HasMember("confirmed"))
			return crawlservpp::Global::Server::CmdResponse(false, true,
	...
	return crawlservpp::Global::Server::CmdResponse("Extracting status reset.");
}

// server command resetanalyzingstatus(urllist): reset the parsing status of a id-specificed URL list
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdResetAnalyzingStatus(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("urllist"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'urllist\' is missing).");
	if(!json["urllist"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Analyzing status reset.");
}

// server command addwebsite(name, namespace, domain): add a website
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdAddWebsite(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("name"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'name\' is missing).");
	if(!json["name"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'name\' is not a string).");
	std::string name = json["name"].GetString();

	if(!json.HasMember("namespace"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'namespace\' is missing).");
	if(!json["namespace"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'namespace\' is not a string).");
	std::string nameSpace = json["namespace"].GetString();

	if(!json.HasMember("domain"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'domain\' is missing).");
	if(!json["domain"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'domain\' is not a string).");
	std::string domain = json["domain"].GetString();

	// check name
	if(!name.length()) return crawlservpp::Global::Server::CmdResponse(true, "Name is empty.");

	// check namespace
	if(nameSpace.length() < 4) return crawlservpp::Global::Server::CmdResponse(true, "Website namespace has to be at least 4 characters long.");
	if(!(this->cmdCheckNameSpace(nameSpace))) return crawlservpp::Global::Server::CmdResponse(true, "Invalid character(s) in website namespace.");
	...
	if(!domain.length()) return crawlservpp::Global::Server::CmdResponse(true, "Domain is empty.");

	// add website to database
	unsigned long id = this->database.addWebsite(name, nameSpace, domain);
	if(!id) return crawlservpp::Global::Server::CmdResponse(true, "Could not add website to database.");

	return crawlservpp::Global::Server::CmdResponse("Website added.", id);
}

// server command updatewebsite(id, name, namespace, domain): edit a website
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdUpdateWebsite(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	while(domain.length() > 7 && domain.substr(0, 8) == "https://") domain = domain.substr(8);
	while(domain.length() && domain.back() == '/') domain.pop_back();
	if(!domain.length()) crawlservpp::Global::Server::CmdResponse(true, "Domain is empty.");
	...
	for(auto i = this->crawlers.begin(); i != this->crawlers.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be changed while crawler is active.");
	for(auto i = this->parsers.begin(); i != this->parsers.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be changed while parser is active.");
	/*for(auto i = this->extractors.begin(); i != this->extractors.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be changed while extractor is active.");
	for(auto i = this->analyzers.begin(); i != this->analyzers.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be changed while analyzer is active.");*/

	// update website in database
	this->database.updateWebsite(id, name, nameSpace, domain);
	return crawlservpp::Global::Server::CmdResponse("Website updated.");
}

// server command deletewebsite(id): delete a website and all associated data from the database by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDeleteWebsite(const rapidjson::Document& json, const std::string& ip) {
	// check whether the deletion of data is allowed
	if(!(this->settings.dataDeletable)) return crawlservpp::Global::Server::CmdResponse(true, "Not allowed.");

	// get argument
	if(!json.HasMember("id")) return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64()) return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is not a number).");
	unsigned long id = json["id"].GetInt64();
	...
	// check whether threads are using the website
	for(auto i = this->crawlers.begin(); i != this->crawlers.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be deleted while crawler is active.");
	for(auto i = this->parsers.begin(); i != this->parsers.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be deleted while parser is active.");
	/*for(auto i = this->extractors.begin(); i != this->extractors.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be deleted while extractor is active.");
	for(auto i = this->analyzers.begin(); i != this->analyzers.end(); ++i)
		if((*i)->getWebsite() == id) return crawlservpp::Global::Server::CmdResponse(true, "Website cannot be deleted while analyzer is active.");*/

	// deletewebsite needs to be confirmed
	if(!json.HasMember("confirmed"))
		return crawlservpp::Global::Server::CmdResponse(false, true, "Do you really want to delete this website?\n"
	...
	return crawlservpp::Global::Server::CmdResponse("Website deleted.");
}

// server command duplicatewebsite(id): Duplicate a website by its ID (no processed data will be duplicated)
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDuplicateWebsite(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	if(!newId) return crawlservpp::Global::Server::CmdResponse(true, "Could not add duplicate to database.");

	return crawlservpp::Global::Server::CmdResponse("Website duplicated.", newId);
}

// server command addurllist(website, name, namespace): add a URL list to the id-specified website
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdAddUrlList(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("website"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'website\' is missing).");
	if(!json["website"].IsInt64())
	...
	// check namespace
	if(nameSpace.length() < 4) return crawlservpp::Global::Server::CmdResponse(true, "Namespace of URL list has to be at least 4 characters long.");
	if(!(this->cmdCheckNameSpace(nameSpace)))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid character(s) in namespace of URL list.");
	if(nameSpace == "config") return crawlservpp::Global::Server::CmdResponse(true, "Namespace of URL list cannot be \'config\'.");
	...
	if(!id) return crawlservpp::Global::Server::CmdResponse(true, "Could not add URL list to database.");

	return crawlservpp::Global::Server::CmdResponse("URL list added.", id);
}

// server command updateurllist(id, name, namespace): edit a URL list
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdUpdateUrlList(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	for(auto i = this->crawlers.begin(); i != this->crawlers.end(); ++i)
		if((*i)->getUrlList() == id) return crawlservpp::Global::Server::CmdResponse(true, "URL list cannot be changed while crawler is active.");
	for(auto i = this->parsers.begin(); i != this->parsers.end(); ++i)
		if((*i)->getUrlList() == id) return crawlservpp::Global::Server::CmdResponse(true, "URL list cannot be changed while parser is active.");
	/*for(auto i = this->extractors.begin(); i != this->extractors.end(); ++i)
		if((*i)->getUrlList() == id) return crawlservpp::Global::Server::CmdResponse(true, "URL list cannot be changed while extractor is active.");
	for(auto i = this->analyzers.begin(); i != this->analyzers.end(); ++i)
		if((*i)->getUrlList() == id) return crawlservpp::Global::Server::CmdResponse(true, "URL list cannot be changed while analyzer is active.");*/

	// update URL list in database
	this->database.updateUrlList(id, name, nameSpace);
	return crawlservpp::Global::Server::CmdResponse("URL list updated.");
}

// server command deleteurllist(id): delete a URL list and all associated data from the database by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDeleteUrlList(const rapidjson::Document& json, const std::string& ip) {
	// check whether the deletion of data is allowed
	if(!(this->settings.dataDeletable)) return crawlservpp::Global::Server::CmdResponse(true, "Not allowed.");
	...
	if(!(this->database.isUrlList(id))) {
		std::ostringstream errStrStr;
		errStrStr << "URL list #" << id << " not found.";
		crawlservpp::Global::Server::CmdResponse(true, errStrStr.str());
	}

	// deleteurllist needs to be confirmed
	if(!json.HasMember("confirmed"))
		return crawlservpp::Global::Server::CmdResponse(false, true, "Do you really want to delete this URL list?\n"
				"!!! All associated data will be lost !!!");
	...
	this->database.log("database", logStrStr.str());
	return crawlservpp::Global::Server::CmdResponse("URL list deleted.");
}

// server command addquery(website, name, query, type, resultbool, resultsingle, resultmulti, textonly): add a query
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdAddQuery(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("website"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'website\' is missing).");
	if(!json["website"].IsInt64())
	...
	if(!json.HasMember("query"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'query\' is missing).");
	if(!json["query"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'query\' is not a string).");
	std::string query = json["query"].GetString();

	if(!json.HasMember("type"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'type\' is missing).");
	if(!json["type"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'type\' is not a string).");
	std::string type = json["type"].GetString();

	if(!json.HasMember("resultbool"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultbool\' is missing).");
	if(!json["resultbool"].IsBool())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultbool\' is not a boolean).");
	bool resultBool = json["resultbool"].GetBool();

	if(!json.HasMember("resultsingle"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultsingle\' is missing).");
	if(!json["resultsingle"].IsBool())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultsingle\' is not a boolean).");
	bool resultSingle = json["resultsingle"].GetBool();

	if(!json.HasMember("resultmulti"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultmulti\' is missing).");
	if(!json["resultmulti"].IsBool())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultmulti\' is not a boolean).");
	bool resultMulti = json["resultmulti"].GetBool();

	if(!json.HasMember("textonly"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'textonly\' is missing).");
	if(!json["textonly"].IsBool())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'textonly\' is not a boolean).");
	...
	// check query text
	if(!query.length()) return crawlservpp::Global::Server::CmdResponse(true, "Query text is empty.");

	// check query type
	if(!type.length()) return crawlservpp::Global::Server::CmdResponse(true, "Query type is empty.");
	if(type != "regex" && type != "xpath") return crawlservpp::Global::Server::CmdResponse(true, "Unknown query type: \'" + type + "\'.");

	// check result type
	if(!resultBool && !resultSingle && !resultMulti) return crawlservpp::Global::Server::CmdResponse(true, "No result type selected.");
	...
	if(!id) return crawlservpp::Global::Server::CmdResponse("Could not add query to database.");

	return crawlservpp::Global::Server::CmdResponse("Query added.", id);
}

// server command updatequery(id, name, query, type, resultbool, resultsingle, resultmulti, textonly): edit a query
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdUpdateQuery(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Query updated.");
}

// server command deletequery(id): delete a query from the database by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDeleteQuery(const rapidjson::Document& json) {
	// check whether the deletion of data is allowed
	if(!(this->settings.dataDeletable)) return crawlservpp::Global::Server::CmdResponse(true, "Not allowed.");
	...
	if(!json.HasMember("confirmed")) return crawlservpp::Global::Server::CmdResponse(false, true, "Do you really want to delete this query?");

	// delete URL list from database
	this->database.deleteQuery(id);

	return crawlservpp::Global::Server::CmdResponse("Query deleted.");
}

// server command duplicatequery(id): Duplicate a query by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDuplicateQuery(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Query duplicated.", newId);
}

// server command testquery(query, type, resultbool, resultsingle, resultmulti, textonly, text): test query on text
void crawlservpp::Global::Server::cmdTestQuery(unsigned long index, const char * body, struct mg_connection * connection) {
	crawlservpp::Global::Server::CmdResponse response;
	...
	if(json.Parse(body).HasParseError()) response = crawlservpp::Global::Server::CmdResponse(true, "Could not parse JSON.");
		...
		// get arguments
		if(!json.HasMember("query"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'query\' is missing).");
		else if(!json["query"].IsString())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'query\' is not a string).");
		else if(!json.HasMember("type"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'type\' is missing).");
		else if(!json["type"].IsString())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'type\' is not a string).");
		else if(!json.HasMember("resultbool"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultbool\' is missing).");
		else if(!json["resultbool"].IsBool())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultbool\' is not a boolean).");
		else if(!json.HasMember("resultsingle"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultsingle\' is missing).");
		else if(!json["resultsingle"].IsBool())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultsingle\' is not a boolean).");
		else if(!json.HasMember("resultmulti"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultmulti\' is missing).");
		else if(!json["resultmulti"].IsBool())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'resultmulti\' is not a boolean).");
		else if(!json.HasMember("textonly"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'textonly\' is missing).");
		else if(!json["textonly"].IsBool())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'textonly\' is not a boolean).");
		else if(!json.HasMember("text"))
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'text\' is missing).");
		else if(!json["text"].IsString())
			response = crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'text\' is not a string).");
		else {
			std::string query = json["query"].GetString();
			std::string type = json["type"].GetString();
			bool resultBool = json["resultbool"].GetBool();
			...
			if(!query.length())
				response = crawlservpp::Global::Server::CmdResponse(true, "Query text is empty.");
			else if(!type.length())
				response = crawlservpp::Global::Server::CmdResponse(true, "Query type is empty.");
			else if(type != "regex" && type != "xpath")
				response = crawlservpp::Global::Server::CmdResponse(true, "Unknown query type: \'" + type + "\'.");
			else if(!resultBool && !resultSingle && !resultMulti)
				response = crawlservpp::Global::Server::CmdResponse(true, "No result type selected.");
			else {
				// test query
				std::string result;
				if(type == "regex") {
					...
					if(!regExTest.compile(query, resultBool || resultSingle, resultMulti))
						crawlservpp::Global::Server::CmdResponse(true, regExTest.getErrorMessage());
					...
					if(resultBool) {
						...
						if(!regExTest.getBool(text, tempResult))
							response = crawlservpp::Global::Server::CmdResponse(true, regExTest.getErrorMessage());
					...
					if(!xPathTest.compile(query, textOnly))
						response = crawlservpp::Global::Server::CmdResponse(true, xPathTest.getErrorMessage());
					else {
						result = "COMPILING TIME: " + timer.tickStr() + "\n";
						if(!xmlDocumentTest.parse(text))
							response = crawlservpp::Global::Server::CmdResponse(true, xmlDocumentTest.getErrorMessage());
						else {
				...
				if(!response.fail){
					result.pop_back();
					response = crawlservpp::Global::Server::CmdResponse(result);
				}
			}
		}
	}
...
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdAddConfig(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("website"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'website\' is missing).");
	if(!json["website"].IsInt64())
	...
	if(!json.HasMember("module"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'module\' is missing).");
	if(!json["module"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'module\' is not a string).");
	...
	if(!json["config"].IsString())
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'config\' is not a string).");
	...
	if(configJson.Parse(config.c_str()).HasParseError()) {
		return crawlservpp::Global::Server::CmdResponse(true, "Could not parse JSON.");
	}
	...
	if(!id) return crawlservpp::Global::Server::CmdResponse(true, "Could not add configuration to database.");

	return crawlservpp::Global::Server::CmdResponse("Configuration added.", id);
}

// server command updateconfig(id, name, config): Update a configuration in the database
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdUpdateConfig(const rapidjson::Document& json) {
	// get arguments
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Configuration updated.");
}

// server command deleteconfig(id): delete a configuration from the database by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDeleteConfig(const rapidjson::Document& json) {
	// check whether the deletion of data is allowed
	if(!(this->settings.dataDeletable)) return crawlservpp::Global::Server::CmdResponse(true, "Not allowed.");
	...
	if(!json.HasMember("confirmed"))
		return crawlservpp::Global::Server::CmdResponse(false, true, "Do you really want to delete this configuration?");

	// delete configuration from database
	this->database.deleteConfiguration(id);

	return crawlservpp::Global::Server::CmdResponse("Configuration deleted.");
}

// server command duplicateconfig(id): Duplicate a configuration by its ID
crawlservpp::Global::Server::CmdResponse crawlservpp::Global::Server::cmdDuplicateConfig(const rapidjson::Document& json) {
	// get argument
	if(!json.HasMember("id"))
		return crawlservpp::Global::Server::CmdResponse(true, "Invalid arguments (\'id\' is missing).");
	if(!json["id"].IsInt64())
	...
	return crawlservpp::Global::Server::CmdResponse("Configuration duplicated.", newId);
}
