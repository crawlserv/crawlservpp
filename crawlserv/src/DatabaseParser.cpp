...
DatabaseParser::DatabaseParser(DatabaseThread& dbThread) : DatabaseModule(dbThread) {
...
void DatabaseParser::initTargetTable(unsigned long websiteId, unsigned long listId, const std::string& websiteNameSpace,
...
// lock URL list
void DatabaseParser::lockUrlList() {
	this->lockTable(this->urlListTable);
}

// check whether an URL has been parsed
bool DatabaseParser::isUrlParsed(unsigned long urlId) {
	sql::ResultSet * sqlResultSet = NULL;
	bool result = false;

	// check prepared SQL statement
	if(!(this->psIsUrlParsed)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::isUrlParsed(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psIsUrlParsed);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::isUrlParsed(...) is NULL");
...
IdString DatabaseParser::getNextUrl(unsigned long currentUrlId) {
	...
	// check prepared SQL statement
	if(!(this->psGetNextUrl)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getNextUrl(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetNextUrl);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getNextUrl(...) is NULL");
...
unsigned long DatabaseParser::getUrlPosition(unsigned long urlId) {
	sql::ResultSet * sqlResultSet = NULL;
	unsigned long result = 0;

	// check prepared SQL statement
	if(!(this->psGetUrlPosition)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getUrlPosition()");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetUrlPosition);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getUrlPosition() is NULL");
...
unsigned long DatabaseParser::getNumberOfUrls() {
	sql::ResultSet * sqlResultSet = NULL;
	unsigned long result = 0;

	// check prepared SQL statement
	if(!(this->psGetNumberOfUrls)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getNumberOfUrls()");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetNumberOfUrls);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getNumberOfUrls() is NULL");
...
bool DatabaseParser::isUrlLockable(unsigned long urlId) {
	sql::ResultSet * sqlResultSet = NULL;
	bool result = false;

	// check prepared SQL statement
	if(!(this->psIsUrlLockable)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::isUrlLockable(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psIsUrlLockable);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::isUrlLockable(...) is NULL");
...
std::string DatabaseParser::getUrlLock(unsigned long urlId) {
	sql::ResultSet * sqlResultSet = NULL;
	std::string result;

	// check prepared SQL statement
	if(!(this->psGetUrlLock)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getUrlLock(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetUrlLock);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getUrlLock(...) is NULL");
...
bool DatabaseParser::checkUrlLock(unsigned long urlId, const std::string& lockTime) {
	sql::ResultSet * sqlResultSet = NULL;
	bool result = false;

	// check prepared SQL statement
	if(!(this->psCheckUrlLock)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::checkUrlLock(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psCheckUrlLock);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::checkUrlLock(...) is NULL");
...
std::string DatabaseParser::lockUrl(unsigned long urlId, unsigned long lockTimeout) {
	// check prepared SQL statement
	if(!(this->psLockUrl)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::lockUrl(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psLockUrl);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::lockUrl(...) is NULL");
...
void DatabaseParser::unLockUrl(unsigned long urlId) {
	// check prepared SQL statement
	if(!(this->psUnLockUrl)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::unLockUrl(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psUnLockUrl);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::unLockUrl(...) is NULL");
...
bool DatabaseParser::getLatestContent(unsigned long urlId, unsigned long index, IdString& contentTo) {
	...
	// check prepared SQL statement
	if(!(this->psGetLatestContent)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getLatestContent(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetLatestContent);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getLatestContent(...) is NULL");
...
std::vector<IdString> DatabaseParser::getAllContents(unsigned long urlId) {
	sql::ResultSet * sqlResultSet = NULL;
	std::vector<IdString> result;

	// check prepared SQL statement
	if(!(this->psGetAllContents)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getAllContents(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetAllContents);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getAllContents(...) is NULL");
	...
	try {
		...
		// get result
		while(sqlResultSet->next()) result.push_back(IdString(sqlResultSet->getUInt64("id"), sqlResultSet->getString("content")));
...
void DatabaseParser::updateOrAddEntry(unsigned long contentId, const std::string& parsedId, const std::string& parsedDateTime,
...
void DatabaseParser::setUrlFinished(unsigned long urlId) {
	// check prepared SQL statement
	if(!(this->psSetUrlFinished)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::setUrlFinished(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psSetUrlFinished);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::setUrlFinished(...) is NULL");
...
unsigned long DatabaseParser::getEntryId(unsigned long contentId) {
	...
	// check prepared SQL statement
	if(!(this->psGetEntryId)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::getEntryId(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psGetEntryId);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::getEntryId(...) is NULL");
...
void DatabaseParser::updateEntry(unsigned long entryId, const std::string& parsedId, const std::string& parsedDateTime,
		const std::vector<std::string>& parsedFields) {
	// check prepared SQL statement
	if(!(this->psUpdateEntry)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::updateEntry(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psUpdateEntry);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::updateEntry(...) is NULL");
...
void DatabaseParser::addEntry(unsigned long contentId, const std::string& parsedId, const std::string& parsedDateTime,
		const std::vector<std::string>& parsedFields) {
	// check prepared SQL statement
	if(!(this->psAddEntry)) throw std::runtime_error("Missing prepared SQL statement for DatabaseParser::addEntry(...)");
	sql::PreparedStatement * sqlStatement = this->getPreparedStatement(this->psAddEntry);
	if(!sqlStatement) throw std::runtime_error("Prepared SQL statement for DatabaseParser::addEntry(...) is NULL");
