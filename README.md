...
The parser and analyzer threads may pre-cache (and therefore temporarily multiply) data in memory, while the crawler and ~~extractor~~ threads work directly on the database, which minimizes memory usage. Because the usual bottleneck for parsers and extractors are requests to the crawled/extracted website, multiple threads are encouraged and considerable multiplication of data is avoided. Multiple threads for parsing and analyzing can be reasonable when using multiple CPU cores, although some additional memory usage by the in-memory multiplication of data should be expected. Of course, in any case a slow database connection or server can have significant impact on performance.
