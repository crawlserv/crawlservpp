...
crawlservpp::Global::Database::Database(const crawlservpp::Struct::DatabaseSettings& dbSettings) : settings(dbSettings) {
	...
	if(!Database::Database::driver) Database::Database::driver = get_driver_instance();
}
...
crawlservpp::Global::Database::~Database() {
	...
	for(auto i = this->preparedStatements.begin(); i != this->preparedStatements.end(); ++i) {
		DATABASE_MAIN_DELETE(i->statement);
	}
...
bool crawlservpp::Global::Database::connect() {
	...
	try {
		...
		if(!sqlStatement) {
			...
			DATABASE_MAIN_DELETE(sqlStatement);
			return false;
		}
...
unsigned long crawlservpp::Global::Database::addThread(const std::string& threadModule, const crawlservpp::Struct::ThreadOptions& threadOptions) {
	...
	try {
		...
		// delete result and SQL statement
		DATABASE_MAIN_DELETE(addStatement);
	}
	catch(sql::SQLException &e) {
		// SQL error
		DATABASE_MAIN_DELETE(addStatement);
...
unsigned long crawlservpp::Global::Database::addWebsite(const std::string& websiteName, const std::string& websiteNameSpace,
	...
	try {
		...
		DATABASE_MAIN_DELETE(createStatement);
...
void crawlservpp::Global::Database::updateWebsite(unsigned long websiteId, const std::string& websiteName, const std::string& websiteNameSpace,
	...
	try {
		...
		if(websiteNameSpace != oldNameSpace) {
			...
			DATABASE_MAIN_DELETE(renameStatement);
			...
			DATABASE_MAIN_DELETE(updateStatement);
		}
		else {
			// create SQL statement for updating
...
unsigned long crawlservpp::Global::Database::duplicateWebsite(unsigned long websiteId) {
	...
	try {
		...
		if(sqlResultSet && sqlResultSet->next()) {
			...
			std::string newNameSpace = Database::Database::duplicateWebsiteNameSpace(websiteNameSpace);
...
void crawlservpp::Global::Database::deleteUrlList(unsigned long listId) {
	...
	try {
		...
		DATABASE_MAIN_DELETE(deleteStatement);
		...
		DATABASE_MAIN_DELETE(dropStatement);
