...
if($website) {
    ...
    if(!$result) exit("ERROR: Could not get URL lists of website.");
    if($result) {
        $first = true;
        while($row = $result->fetch_assoc()) {
            $ulId = $row["id"];
    ...
    if($urllist) {
        ...
        echo " data-website-namespace=\"".htmlspecialchars($namespace)."\" data-namespace=\"".htmlspecialchars($urllistNamespace)
                ."\">[Download]</a>\n";
        echo "</div>\n";
        echo "</div>\n";
        if($urllistCrawled) {
