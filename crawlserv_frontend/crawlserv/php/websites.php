...
if(isset($website)) {
    if($website) {
        ...
        $result = $dbConnection->query(
                "SELECT namespace,domain,dir FROM crawlserv_websites WHERE id=$website LIMIT 1"
        );
        
        if(!$result)
            exit("ERROR: Could not get namespace and domain of website.");
...
if($website) {
    ...
    $result = $dbConnection->query(
            "SELECT id,name,namespace FROM crawlserv_urllists WHERE website=$website ORDER BY name"
    );
    
    if(!$result)
        exit("ERROR: Could not get URL lists of website.");
    ...
    if(isset($urllist) && $urllist) {
        ...
        $result = $dbConnection->query(
                "SELECT COUNT(*) FROM `crawlserv_".$namespace."_".$urllistNamespace."`"
        );
        
        if(!$result)
            exit(
                "ERROR: Could not get size of URL list from `crawlserv_".$namespace."_".$urllistNamespace."`"
            );
        ...
        $result = $dbConnection->query(
                "SELECT COUNT(DISTINCT url) FROM `crawlserv_".$namespace."_".$urllistNamespace
                ."_crawling` WHERE success = TRUE"
        );
        
        if(!$result)
            exit("ERROR: Could not get number of crawled URLs from `crawlserv_".$namespace."_".$urllistNamespace."_crawling`");
        ...
        if(!$result)
            exit("ERROR: Could not get number of parsed URLs from `crawlserv_".$namespace."_".$urllistNamespace."_parsing`");
        ...
        if(!$result)
            exit(
                "ERROR: Could not get number of extracted URLs from `crawlserv_".$namespace."_".$urllistNamespace."_extracting`"
            );
        ...
        $result = $dbConnection->query(
                "SELECT MAX(crawltime) AS updated FROM `crawlserv_".$namespace."_".$urllistNamespace."_crawled`"
        );
        ...
        $result = $dbConnection->query(
                "SELECT updated FROM crawlserv_parsedtables WHERE website=$website AND urllist=$ulId ORDER BY updated DESC LIMIT 1"
        );
        ...
        $result = $dbConnection->query(
                "SELECT updated FROM crawlserv_extractedtables WHERE website=$website AND urllist=$ulId ORDER BY updated  DESC LIMIT 1"
        );
    ...
    if(!$urllist)
        echo "<option value=\"0\" selected>Add new</option>\n";
    else
