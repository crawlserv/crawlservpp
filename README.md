...
The server needs a configuration file as argument, the test configuration can be found at [`crawlserv/config`](crawlserv/config). Replace the values in this file with those for your own database. The password for granting the server full access to the database will be prompted when starting the server.
